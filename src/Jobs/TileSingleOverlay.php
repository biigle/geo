...
class TileSingleOverlay extends TileSingleObject
{
    ...
    public function generateTiles($file, $path)
    {
        ...
        $noDataValue = $this->exif['IFD0:GDALNoData'] ?? null;

        // Check if the minimum is the real minimum and not the noDataValue
        if ($noDataValue !== null) {
            $minIsNoDataValue = $noDataValue == $min || $min != 0 && $noDataValue / $min >= 0.999;
            ...
            $noDataValue = $minIsNoDataValue ? $min : $noDataValue;
            $min = $this->getNextMin($min, $noDataValue, $minIsNoDataValue);
        }

        $max = $this->vipsImage->max();
        $isBlackImg = $min == 0 && $max == 0;

        if (!$isBlackImg && $this->vipsImage->bands === 1) {
            // Must be called before imageNormalization since pixel values could be changed
            $alpha = $this->maybeGenerateAlphaMask($noDataValue);

            // If a smaller or larger range is used, normalize the pixel range to the enhance contrast
        ...
        // Add bit depth to prevent segmentation faults when processing float images
     ...
     * @return float
     */
    protected function getNextMin($min, $noDataValue, $minIsNoDataValue)
    {
        if (!$minIsNoDataValue && $noDataValue >= $min) {
            return $min;
        }
     ...
     */
    protected function maybeGenerateAlphaMask($noDataValue)
    {
        if ($noDataValue === null) {
            return;
        }
