<template>
    <modal v-model="show" title="Map Filter" size="lg" :backdrop="false" @hide="callback">
        <div class="content">
            <div class="cell cell-map">
                <image-map v-if="showOverlay" :images="images" :selectable="true" v-on:select="handleSelectedImages"
                    :overlays="geoOverlays" :overlay-url-template="overlayUrlTemplate" :last-action="lastAction"
                    :hide-ids="hideIds"></image-map>
            </div>
            <div class="cell cell-edit">
                <div v-if="geoOverlays.length === 0">
                    <button class="layer-button" @click="showLayers = !showLayers"
                        title="Show available geo-overlays"><i class="fas fa-layer-group"></i></button>
                    <div class="layers" :class="{ active: showLayers }">
                        <h4>Geo Overlays</h4>
                        <p class="text-danger">Currently no overlays uploaded / selected.</p>
                        <p class="text-muted">
                            <em>Hint:</em> In the volume-edit section, you can upload a geo-overlay and mark it as
                            browsing-layer to make it available here.
                        </p>
                    </div>
                </div>
                    ...
                    <div class="layers" :class="{ active: showLayers }">
                        ...
                        <div v-if="geoOverlays.length !== 0">
                            ...
                            <div v-for="overlay in geoOverlays" :key="overlay.id">
                                <button :id="overlay.id" class="list-group-item custom"
                                    v-on:click="setLastAction(overlay.id)" :disabled="isDisabled(overlay.id)">
            ...
            <div class="cell cell-content">
                <p class="text-muted">
                    <em>Hint:</em> Select image locations on the volume map by drawing an encompassing rectangle. To do
                    this, press and hold <kbd>Ctrl</kbd> (<kbd>Cmd &#8984;</kbd> on Mac) as well as the left mouse
