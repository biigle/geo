...
<div class="row">
    ...
    <p>
        If images of a volume contain the geo location in their <a href="{{route('manual-tutorials', ['volumes', 'image-metadata'])}}">metadata</a>, they can be disblayed on the volume map. For these volumes the <button class="btn btn-default btn-xs"><i class="glyphicon glyphicon-globe"></i></button> button appears in the sidebar of the volume overview. Click this button to view the volume map.
    </p>
    <p class="text-center">
    ...
    <p>
        The locations of the images are displayed as dots on the map. Grab and move the map with the cursor to pan around or use the mouse wheel to zoom. Use the <i class="glyphicon glyphicon-resize-small"></i> button to fit the view back to the viewport that shows the locations of all images.
    </p>
    <h3>Annotation filter</h3>
    <p>
        You can filter the displayed locations by annotations that the images contain. This allows you to explore the spatial occurrence of different species or objects that you annotated earlier. Open the label trees tab at the right with a click on the <button class="btn btn-default btn-xs"><i class="glyphicon glyphicon-tags"></i></button> button. Now select a label from the label trees. The displayed locations will immediately update to show only images that contain annotations which have the selected label attached. You can also select multiple labels at the same time. The map will show the locations of images where <em>any</em> of the selected labels occur.
    </p>
    <p class="text-center">
